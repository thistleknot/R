...
newDF <- training[,"MSPUS",drop=FALSE]

for (n in 1:length(names))
{
  newDF <- cbind(newDF,dplyr::lead(training[,names[n],drop=FALSE],n=abs(lags[n])))
...
lm_ <- lm(MSPUS ~., data=na.omit(newDF))
...
error_1 <- mean(abs(holdout[,var_of_int][1:nrow(f_1)]-f_1[,1]))

test_2 <- auto.arima(lm_$model[,var_of_int,drop=FALSE], xreg = as.matrix(lm_$model[,2:ncol(newDF)]))

f_2 <- as.data.frame(forecast(test_2, h=horizon, xreg = as.matrix(holdout[1:horizon,colnames(newDF)[2:length(colnames(newDF))]])))

rownames(f_2) <- as.integer(rownames(f_2))+abs(as.integer(rownames(f_2))-nrow(training))[1]+1

error_2 <- mean(abs(holdout[,var_of_int][1:nrow(f_2)]-f_2[,1]))
