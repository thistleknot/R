...
df <- tail(data.frame(time = anydate(rownames(actual_2)), cbind(actual_2,mean_,lower_,upper_)),horizon*2)
...
rownames(properSet) <- anydate(d_s)
...
t_ <- structure(data.frame(c(combo_[,var_of_int]), row.names = rownames(combo_)),names=var_of_int)


nv_diff_sets(var_of_int,t_,c())

na.omit(structure(data.frame(c(combo_[,var_of_int]), row.names = rownames(combo_)),names=var_of_int))

structure(data.frame(inv_d, row.names = rownames(properSet)),names=var_of_int)
...
lower_ <- nv_diff_sets(var_of_int,,forecasts_a[which.min(errors)][[1]][,4])
upper_ <- nv_diff_sets(var_of_int,,forecasts_a[which.min(errors)][[1]][,5])
#nrow(actual)
length(mean_)
