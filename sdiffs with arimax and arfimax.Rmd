...
#packages
{
packages <- c("dplyr","ggplot2","quantmod","zoo","plyr","forecast","tseries","tidyverse","furrr","lubridate","anytime","car","caret","ppcor","TSA","corrplot")
...
differences <- as.data.frame(rbind(sndif_[which(match(colnames(raw),c(var_of_int,winners))>0)],ndif_[which(match(colnames(raw),c(var_of_int,winners))>0)]))
colnames(differences) <- c(var_of_int,winners)

rownames(differences) <- c("seasonal","nonseasonal")
differences
