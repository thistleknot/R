...
#packages
{
packages <- c("dplyr","ggplot2","quantmod","zoo","plyr","forecast","tseries","tidyverse","furrr","lubridate","anytime","car","caret","ppcor","whitening","TSA","corrplot")
...
for (n in 1:(length(colnames(combo_s))))
{
  #print(n)
  d_ <- ndiffs(combo_s[,n])
  #min 1 to ensure I'm being consisten (i.e. measuring rate of change between quarters)
  if(d_ == 0)
  {
    d_ = 1
    }
...
#CFNAIDIFF has negative initial values which make calculating returns impossible
{
  ...
  {#d=28
    ...
    }else
    {
      ...
      for (sdif in 1:sndif_[d])
      {
        #print(sdif)
        #View(temp)
        temp <- (1+((temp-dplyr::lag(temp,1*seasonal))/dplyr::lag(temp,1*seasonal)))
      }
      combo_s_m <- cbind(combo_s_m,temp)    
    }
  ...
  #View(raw[,"GVZCLS",drop=FALSE])
  #View(combo_s_m)
  
  #hist(combo_s_m$MABMM301USM189S)
  ...
  {#d=53
    ...
    if(ndif_[d] == 0)
    {
      ...
      temp <- as.data.frame(unlist(1+lapply((temp-dplyr::lag(temp,1)),function(x){ifelse(is.nan(x),return(0),return(x))})))
      colnames(temp) <- colnames(combo_s_m[,d,drop=FALSE])
      rownames(temp) <- rownames(combo_s_m[,d,drop=FALSE])
      combo_d_m <- cbind(combo_d_m,temp)
    }else
    '
    {
      temp <- combo_s_m[,d,drop=FALSE]
      ...
      {
        temp <- as.data.frame(unlist(1+lapply((temp-dplyr::lag(temp,1)),function(x){ifelse(is.nan(x),return(0),return(x))})))
          #(1+((temp-dplyr::lag(temp,1))/dplyr::lag(temp,1)))
      }
...
'
ndif_[match("CFNAIDIFF",colnames(combo_s_m))]
sndif_[match("CFNAIDIFF",colnames(combo_s_m))]

hist(combo_d_m$CFNAIDIFF)

colnames(combo_d_m[,(ndif_ + sndif_)>0])

combo_d_m[,(ndif_ + sndif_)>0] <- log10(combo_d_m[,(ndif_ + sndif_)>0])
'
#View(combo_d_m)
...
{#c=3
  ...
  ccf1 <- ccf(training[,var_of_int,drop=FALSE],training[,c], lag.max = seasonal, correlation=TRUE, plot=FALSE)
...
corrplot(PCOR(subset[,c(var_of_int,winners)]))
'
...
rownames(differences) <- c("lags","seasonal","nonseasonal")
print(differences)
