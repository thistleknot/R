...
nv_diff_sets <- function(var_of_int,dataset,f_casts)
{
  ...
  #-1?, based on d_?
  startRow = which(rownames(raw)==rownames(dataset[1:d_,]))
...
actual_2 <- tail(as.data.frame(diffinv(c(na.omit(actual_[,1])),differences=d_,xi=raw[which(rownames(actual_)[1]==rownames(raw)),var_of_int])),-1)

#actual_2

#raw[,var_of_int,drop=FALSE]

colnames(actual_2) <- colnames(actual_)
rownames(actual_2) <- head(rownames(actual_),-1)

df <- tail(data.frame(time = anydate(rownames(actual_2)[1:length(mean_)]), actual_2[1:length(mean_),1], mean_,lower_,upper_),horizon*2)
...
forecasts_a <- list(
  ...
  f_4 <- as.data.frame(forecast(models[[6]]))
