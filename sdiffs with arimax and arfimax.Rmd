...
newDF_h <- newDF[(nrow(training)+1):nrow(combo_),]
...
#min of 1
differences <- as.data.frame(rbind(c(0,lags[which(match(names,c(var_of_int,winners))>0)]),sndif_[which(match(colnames(raw),c(var_of_int,winners))>0)],ndif_[which(match(colnames(raw),c(var_of_int,winners))>0)]))
...
models <- list(
  ...
  #auto.arima(lm_$residuals),
  #arfima(lm_$residuals),
  #auto.arima(newDF_t[,var_of_int,drop=FALSE],xreg=lm_$residuals),
...
forecasts <- list(
  ...
  if(FALSE)
  {
    ...
    f_1 <- as.data.frame(forecast(models[[3]],h=horizon))
  ...
  {
    f_4 <- as.data.frame(forecast(models[[2]],h=horizon))
    rownames(f_4) <- rownames(actual)
    f_4
  },
  {
    #proper arimax
    f_5 <- as.data.frame(forecast(models[[3]], xreg=as.matrix(newDF_h[1:horizon,winners,drop=FALSE]), h=horizon))
...
errors <- list(
  ...
  #mean(abs(forecasts[[2]]$`Point Forecast`-t(actual))),
  #mean(abs(forecasts[[3]]$`Point Forecast`-t(actual)))
  #mean(abs(forecasts[[4]]$`Point Forecast`-t(actual))),
...
nv_diff_sets <- function(var_of_int,dataset,f_casts)
{
  ...
  startRow = which(rownames(raw)==rownames(dataset[1:d_,]))-1
  
  data_ <- na.omit(c(dataset[,var_of_int], f_casts))
...
actual_2 <- as.data.frame(tail(diffinv(actual_[,1],differences=d_,xi=raw[which(rownames(actual_)[1]==rownames(raw))-1,var_of_int]),-1))

colnames(actual_2) <- colnames(actual_)
rownames(actual_2) <- rownames(actual_)

df <- tail(data.frame(time = anydate(rownames(actual_2)[1:length(mean_)]), actual_2, mean_,lower_,upper_),horizon*2)
...
fore_data <- na.omit(newDF[,c(var_of_int,winners)])
...
models_a <- list(
  ...
  auto.arima(lm_a$residuals),
  arfima(lm_a$residuals),
  auto.arima(fore_data[,var_of_int,drop=FALSE],xreg=lm_a$residuals),
...
s_=sndif_[which(colnames(raw)==var_of_int)]
d_=ndif_[which(colnames(raw)==var_of_int)]
data_ <- c(na.omit(c(raw[,var_of_int],forecasts_a[which.min(errors)]$`Point Forecast`)))
d_set_ <- (which(rownames(raw)==rownames(raw[1,,]))):(which(rownames(raw)==rownames(newDF_t[1,,]))+(d_-1))
...
diffinv(newDF_t[,var_of_int,drop=TRUE],differences=d_,xi=raw[rownames(newDF_t[1,]),var_of_int])

diffinv(newDF_t[,var_of_int],differences=d_)

raw[,var_of_int]

properSet <- as.data.frame(c(matrix(NA,nrow=(nrow(raw)-length(inv_d))),inv_d))
colnames(properSet) <- colnames(raw[,var_of_int,drop=FALSE])
rownames(properSet) <- rownames(raw)

mean_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(forecasts_a[[which.min(errors)]]$`Point Forecast`))
lower_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(forecasts_a[[which.min(errors)]]$'Lo 95'))
upper_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(forecasts_a[[which.min(errors)]]$'Hi 95'))

#actual_f <- raw[rownames(actual),var_of_int,drop=FALSE]

newDates <- c(anydate(rownames(newDF))[length(rownames(newDF))]+days(92),
anydate(rownames(newDF))[length(rownames(newDF))]+days(91*2),
anydate(rownames(newDF))[length(rownames(newDF))]+days(91*3),
anydate(rownames(newDF))[length(rownames(newDF))]+days(91.25*4),
anydate(rownames(newDF))[length(rownames(newDF))]+days(91.4*5)
)

#lubridate::ceiling_date(Sys.Date(), unit = "month")
      
df_a <- data.frame(time = newDates, mean_,lower_,upper_)
