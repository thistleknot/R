...
x = x_[,2:ncol(x_)]
corrplot(cor(cbind(x_[,1,drop=FALSE],as.data.frame(ZCA_svd(x)))))
py$svd_whiten(x_[,2:ncol(x_)])

np <- import("numpy")

np$dot(py$zca_whitening_matrix(as.matrix(x_[,2:ncol(x_)])),as.matrix(x_[,2:ncol(x_)]))
py$white(x_[,2:ncol(x_)])
...
subset_w <- cbind(subset[,var_of_int,drop=FALSE],as.data.frame(py$white(as.matrix(subset[,c(colnames(newDF_t) %notin% c(var_of_int))]))))
...
mean_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF_t),var_of_int,drop=FALSE],season,c(f_3a$`Point Forecast`))
lower_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF_t),var_of_int,drop=FALSE],season,c(f_3a$'Lo 95'))
upper_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF_t),var_of_int,drop=FALSE],season,c(f_3a$'Hi 95'))
...
#length(lm_a$residuals)
#View(lm_a$residuals)
#nrow(na.omit(newDF[,c(var_of_int,winners)]))

#plot(lm_a$residuals)

forecast_a <- auto.arima(fore_data[,var_of_int,drop=FALSE],xreg=lm_a$residuals)
newDF[,c(var_of_int,winners)]

#h=max(differences[1,2:length(colnames(differences))
f_3_lm_r_a <- forecast(lm_a, fore_data, h=horizon)

#test_4 <- arfima(newDF_t[,var_of_int,drop=FALSE],xreg=lm_$residuals,estim=c("mle"))

f_3a_a <- as.data.frame(forecast(forecast_a,h=horizon,xreg=f_3_lm_r_a$mean))


mean_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(f_3a$`Point Forecast`))
lower_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(f_3a$'Lo 95'))
upper_ <- nv_diff_sets(sndif_[which(colnames(raw)==var_of_int)],ndif_[which(colnames(raw)==var_of_int)],raw[rownames(newDF),var_of_int,drop=FALSE],season,c(f_3a$'Hi 95'))
