...
#do cumsum to find out the reconstructed value at any given position
reconstruct <- do.call(cbind,lapply(1:length(ndif_),function(d)
{#d=8
  ...
  rownames(set) <- rownames(combo_d)
  
  reconstructed <- as.data.frame(c(unlist(lapply(rbind(1:(ncol(set)-1)),function(r_)
  {#r_=3
    (set[r_,r_])
  })),set[(ndif_[d]+1):nrow(set),ncol(set)]))
  
  colnames(reconstructed)<-colnames(raw[,d,drop=FALSE])
  rownames(reconstructed) <- rownames(combo_d)
