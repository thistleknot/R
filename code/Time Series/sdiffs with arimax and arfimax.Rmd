...
actual_h <- newDF_h[1:horizon,var_of_int,drop=FALSE]
...
forecasts <- list(
  {
    ...
    rownames(f_0) <- rownames(actual_h)
    f_0
  },
  if(FALSE)
  {
    ...
    rownames(f_1a) <- rownames(actual_h)
    f_1a
  }else
  {
    NA  
  ...
  if(FALSE)
  {
    ...
    rownames(f_2a) <- rownames(actual_h)
    f_2a
  }else
  {
    NA
  ...
  if(FALSE)
  {
    ...
    #rownames(f_3a)
    rownames(f_3a) <- rownames(actual_h)
    f_3a
  }else
    {
      NA
  ...
  ,
  {
    ...
    rownames(f_4) <- rownames(actual_h)
    f_4
  },
  {
    #proper arimax
    f_5 <- as.data.frame(forecast(models[[6]], xreg=as.matrix(newDF_h[1:horizon,winners,drop=FALSE]), h=horizon))
    rownames(f_5) <- rownames(actual_h)
    f_5
  }
)

errors <- list(
  mean(abs(forecasts[[1]]$`Point Forecast`-t(actual_h))),
  NA,#mean(abs(forecasts[[2]]$`Point Forecast`-t(actual_h)))
  NA,#mean(abs(forecasts[[3]]$`Point Forecast`-t(actual_h))
  NA,#mean(abs(forecasts[[4]]$`Point Forecast`-t(actual_h)))
  mean(abs(forecasts[[5]]$`Point Forecast`-t(actual_h))),
  mean(abs(forecasts[[6]]$`Point Forecast`-t(actual_h)))
)
