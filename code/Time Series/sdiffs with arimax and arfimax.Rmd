...
models_a <- list(
  ...
  #xreg=structure(data.frame(const=matrix(1,nrow=nrow(fore_data)),fore_data[,winners]),names=c("constant",winners))
...
forecasts_a <- list(
  ...
  {
    #proper arimax
    f_5 <- as.data.frame(forecast(models_a[[6]], xreg=as.matrix(head(tail(fore_data[,winners,drop=FALSE],(season-1)),horizon))))
