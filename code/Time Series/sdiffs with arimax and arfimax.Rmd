...
#(index(ts_)-index(ts(combo_[,var_of_int,drop=FALSE],frequency=season, start = c(year(rownames(combo_)[1]),(month(rownames(combo_)[1])/12)*4), )))*12


#d_s <- eom(c(anydate(rownames(raw)[1:edge]),anydate(rownames(raw)[1]) %m+% months((index(ts_)-year(anydate(rownames(raw)[1])))*12)))
...
ndif_[which(colnames(raw)==winners)]
sndif_[which(colnames(raw)==winners)]

sndif_[which(colnames(raw)==var_of_int)]
ndif_[which(colnames(raw)==var_of_int)]

tail(fore_data)

head(fore_data)

View(cbind(combo_[,winners],fore_data[,winners]))

View(fore_data[,,drop=FALSE])

write.csv(fore_data, "fore.csv", row.names = TRUE)
write.csv(combo_[,winners,drop=FALSE], "combo_.csv", row.names = TRUE)
#write.csv(fore_data)

combo_[,c(var_of_int,winners),]
...
{#n=236
  ...
  } else
  {
    #n=1
    #print(n)
    #ts(raw[,var_of_int],frequency = season)
    ...
    rownames(temp) <- eom(anydate(rownames(raw[1,,drop=FALSE])) %m+% months(qtrs*12))
